...
class TemporaryDisease extends StatefulWidget {
  @override
  _TemporaryDiseasePageState createState() => _TemporaryDiseasePageState();
}

class _TemporaryDiseasePageState extends State<TemporaryDisease> {
  /// 头部数据
...
//  final GlobalKey<FormState> _formKey = GlobalKey<FormState>();
  ...
  _getDiseaseWayModel() async {
    DiseaseWayModelEntityEntity diseaseWayModelEntityEntity = await DiseaseReportNetWorkQuery.diseaseRooadList();
    List<String> tempList = [];
    for(DiseaseWayModelEntityResult model in diseaseWayModelEntityEntity.result){
     tempList.add(model.name);
    }
    setState(() {
      ways.addAll(tempList);
    });
  }
...
//            key: _formKey,
  ...
  /// 检测数据是否缺少
  _checkData(){
    if(inputHeaderValueMaps['发现时间：'] == null){
    ...
    for (int i = 0; i < _sectionCount; i++) {
      ...
      if(tempMap != null){
      if(tempMap['病害种类：'] == null){
        Fluttertoast.showToast(msg: "请选择病害种类");
        return false;
      }if(tempMap['病害数量：'] == null){
        Fluttertoast.showToast(msg: "请输入病害数量");
        return false;
      }
      if(tempMap['病害位置：'] == null){
        Fluttertoast.showToast(msg: "请选择病害位置");
        return false;
      }
      if(tempMap['原因分析：'] == null){
        Fluttertoast.showToast(msg: "请选择病害原因");
        return false;
      }
      if(tempMap['整修措施：'] == null){
        Fluttertoast.showToast(msg: "请选择整修措施");
        return false;
      }
      }else
        {
          Fluttertoast.showToast(msg: "请填写病害描述");
          return false;
        }
    }
    return true;
